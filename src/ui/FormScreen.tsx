...
import { KeyboardAvoidingView, Platform, ScrollView, StyleSheet, View, type LayoutChangeEvent } from 'react-native';
 ...
 *   layering padding on top of that double-counts the keyboard. NOTE
 *   (measured on a Pixel_9 / Android 15 emulator, M4-T3): under the
 *   edge-to-edge enforcement that Android 15 applies to `targetSdk` 35+, the
 *   window is *not* resized for the IME any more, and `KeyboardAvoidingView`
 *   with `behavior="padding"` does not compensate either — the keyboard
 *   covers the pinned footer. That is a platform-level gap needing real IME
 *   `WindowInsets` handling, unchanged from before this component existed and
 *   out of scope here; it is written up in the M4-T3 report.
...
export function FormScreen({ title, back, right, footer, children }: FormScreenProps) {
  ...
  const clearance = useBottomClearance({ extra: footerHeight });
    ...
    <Screen title={title} back={back} right={right}>
      ...
      {footer ? (
        ...
        <View
          ...
          style={[styles.footer, { backgroundColor: theme.colors.bg, paddingBottom: insets.bottom + space[3] }]}
        >
          {footer}
        </View>
      ) : null}
